...
const anim = (y = 16, d = 0) => ({ initial: { y, opacity: 0 }, whileInView: { y: 0, opacity: 1 }, viewport: { amount: 0.6, once: false }, transition: { duration: DUR, ease: EASE, delay: d } });
const enter = { initial: { opacity: 0, y: 36, scale: 0.98 }, whileInView: { opacity: 1, y: 0, scale: 1 }, viewport: { amount: 0.6, once: false }, transition: { duration: DUR, ease: EASE } };
const enterCard = (d = 0) => ({ initial: { y: 28, opacity: 0, scale: 0.96 }, animate: { y: 0, opacity: 1, scale: 1 }, transition: { duration: DUR, ease: EASE, delay: d } });
const ytId = (url) => { try { const u = new URL(url); if (u.hostname.includes("youtu.be")) return u.pathname.slice(1); if (u.hostname.includes("youtube.com")) return u.searchParams.get("v"); } catch {} return null; };
    ...
    <div className="resources">
      ...
      <div ref={viewportRef} className="resources__viewport">
        ...
        <section ref={(el) => (sectionRefs.current[0] = el)} className="section section--intro snap-start">
          <div className="intro">
            <motion.div initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: EASE }}>
        ...
        {/* Secciones */}
        {SECTIONS.map((s, i) => (
          <section key={s.key} ref={(el) => (sectionRefs.current[i + 1] = el)} className="section snap-start">
            <motion.div className="panel" {...enter}>
              <motion.h2 className="panel__title" {...anim(16, 0.05)}>{s.title}</motion.h2>
              ...
              <div className="panel__nav">
                ...
                {i < SECTIONS.length - 1 && <button className="panel__btn" onClick={() => scrollTo(i + 2)}>Next →</button>}
              </div>
            </motion.div>
          </section>
        ))}
